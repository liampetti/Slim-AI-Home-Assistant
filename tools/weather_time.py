...
def load_weather_config():
    ...
    config_path = os.path.join(os.path.dirname(__file__), 'config', 'weather_data.json')
    try:
        with open(config_path, 'r') as f:
            config = json.load(f)
        return config['ftp']
    except Exception as e:
        # Fallback to default values (weather data for NSW, Australia) if config file is not available
        return {
            "host": "ftp.bom.gov.au",
